...
private:
    int _countRangeSum(vector<int>& nums, int64_t lower, int64_t upper, int64_t l, int64_t r) {
        if (r == l) return lower <= nums[l] && nums[l] <= upper;

        int m = (l + r) >> 1;
        int ans = _countRangeSum(nums, lower, upper, l, m) + _countRangeSum(nums, lower, upper, m + 1, r);

        vector<int64_t> l_presum, r_presum;
        for (int i = m; i >= l; i--) l_presum.push_back(nums[i]);
        for (int i = m + 1; i <= r; i++) r_presum.push_back(nums[i]);
        ...
        while (ll < r_size && l_presum[0] + r_presum[ll] < lower) ll++;
        while (rr >= 0 && l_presum[0] + r_presum[rr] > upper) rr--;
        ll--; rr++;
        ans += rr - ll - 1;
        ...
        for (int i = 1; i < l_size; i++)
        {
            while (ll >= 0 && l_presum[i] + r_presum[ll] >= lower) ll--;
            while (rr < r_size && l_presum[i] + r_presum[rr] <= upper) rr++;
            ans += rr - ll - 1;
        };
        return ans;
    };
