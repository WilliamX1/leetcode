...
public:
    ...
    int nearestExit(vector<vector<char>>& maze, vector<int>& entrance) {
        ...
        vector<vector<int>> v(m, vector<int>(n, 0x7fffffff)); v[entrance[0]][entrance[1]] = 0;
        queue<node> q; q.push(node(entrance[0], entrance[1], 0));

        while (!q.empty())
        {
            node cur = q.front(); q.pop();
            if ((cur.x != entrance[0] || cur.y != entrance[1]) && (cur.x == 0 || cur.x == m - 1 || cur.y == 0 || cur.y == n - 1)) return cur.cnt;
