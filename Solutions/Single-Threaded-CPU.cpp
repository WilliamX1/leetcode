...
public:
    struct task {
        ...
        const bool operator< (const task& b) const {
            if (this->end_time - this->begin_time != b.end_time - b.begin_time) return this->end_time - this->begin_time > b.end_time - b.begin_time;
            else return this->index > b.index;
        }
    };
    vector<int> getOrder(vector<vector<int>>& tasks) {
        ...
        _queue.push(_task.back()); _task.pop_back(); //将第一个任务压入栈
