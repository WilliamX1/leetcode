...
public:
    vector<int> gridIllumination(int n, vector< vector<int> >& lamps, vector< vector<int> >& queries) {
        ...
        for (int i = 0; i < queries.size(); i++) {
            ...
            vector<int> index; index.push_back(row); index.push_back(col);
            if (rows[row] > 0 || cols[col] > 0 || left_point[row + col] > 0 || right_point[row - col] > 0) ans.push_back(1);
            else ans.push_back(0);

            for (int j = -1; j <= 1; j++)
                for (int k = -1; k <= 1; k++) {
                    vector<int> tmp; tmp.push_back(row + j); tmp.push_back(col + k);
                    if (s.find(tmp) != s.end()) {
                        s.erase(s.find(tmp));
                        rows[row + j]--;
                        cols[col + k]--;
