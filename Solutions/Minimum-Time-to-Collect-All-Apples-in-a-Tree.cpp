class Solution {
public:
    int getApples(unordered_map<int, vector<int>>& sons, const vector<bool>& hasApple, vector<bool>& havebeen, const int& root) {
        havebeen[root] = true;
        vector<int> son = sons[root];
        if (son.empty()) return hasApple[root] ? 0 : -1;
        else {
            int tot = hasApple[root] ? 0 : -1;
            for (const int& s : son) {
                if (havebeen[s]) continue;
