...
{
  public:
    int _perfectMenu(vector<int> materials, const vector<vector<int>> &cookbooks, const vector<vector<int>> &attribute,
                     int index, int n, int cur, int limit)
    {
        int ans = -1;
        if (limit <= 0)
            ans = cur;
        ...
        ans = max(ans, _perfectMenu(materials, cookbooks, attribute, index + 1, n, cur + attribute[index][0],
