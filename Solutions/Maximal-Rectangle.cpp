    ...
    int maximalRectangle(vector< vector<char> >& matrix) {
        ...
        for (int i = 0; i < rows; i++)
            for (int j = 0; j < cols; j++) {
                if (i == 0) col_seq[i][j] = matrix[i][j] == '1';
                else col_seq[i][j] = matrix[i][j] == '1' ? col_seq[i - 1][j] + 1 : 0;
                if (j == 0) row_seq[i][j] = matrix[i][j] == '1';
                else row_seq[i][j] = matrix[i][j] == '1' ? row_seq[i][j - 1] + 1 : 0;
