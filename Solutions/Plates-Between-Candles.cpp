...
public:
    vector<int> platesBetweenCandles(string s, vector<vector<int>>& queries) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            // cout << right << ' ' << left << ' ' << star_backward[right] << ' ' << star_forward[left] << ' ' << line[right] << ' ' << line[left - 1] << endl;
            int line_cnt = line[right] - (left ? line[left - 1] : 0);
            if (line_cnt >= 2) answer[i] = (right - left + 1) - star_backward[right] - star_forward[left] - (line[right] - (left ? line[left - 1] : 0));
            else answer[i] = 0;
        };
