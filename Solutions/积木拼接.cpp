...
public:
        ...
        for (int y = 0; y < n; y++) {
            for (int z = 0; z < n; z++) {
                ...
                if (ch == 0) {
                    yy = y;
                    zz = z;
                } else if (ch == 1) { // left-right
                    yy = n - y - 1;
                    zz = z;
                } else if (ch == 2) { // up-down
                    yy = y;
                    zz = n - z - 1;
                } else if (ch == 90) {
                    yy = z, zz = n - y - 1;
    ...
    bool dfs(const vector<vector<string>>& shapes, const int& index, const vector<vector<vector<bool>>>& havebeen) {
        ...
        for (const int& ch : choice) {
            have = havebeen;
            if (OK_X(shapes[index], have, x, ch) && dfs(shapes, index + 1, have)) return true;
        };
        x = n - 1;
        for (const int& ch : choice) {
            have = havebeen;
        ...
        for (const int& ch : choice) {
            have = havebeen;
            if (OK_Y(shapes[index], have, y, ch) && dfs(shapes, index + 1, have)) return true;
        };
        y = n - 1;
        for (const int& ch : choice) {
            have = havebeen;
        ...
        for (const int& ch : choice) {
            have = havebeen;
            if (OK_Z(shapes[index], have, z, ch) && dfs(shapes, index + 1, have)) return true;
        };
        z = n - 1;
        for (const int& ch : choice) {
            have = havebeen;
