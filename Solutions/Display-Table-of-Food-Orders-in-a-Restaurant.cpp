...
class Solution
{
  public:
    vector<vector<string>> displayTable(vector<vector<string>> &orders)
    {
        ...
        food_name.push_back("Table");

        int len = orders.size();
        unordered_map<string, int> food_to_int;
        int food_cnt = 1;
        unordered_map<string, int> table_to_int;
        int table_cnt = 0;

        /* 按照桌号排序 */
        sort(orders.begin(), orders.end(),
             [](const vector<string> &a, const vector<string> &b) { return stoi(a[1]) < stoi(b[1]); });
        ...
        /* 按照菜名排序 */
        sort(orders.begin(), orders.end(),
             [](const vector<string> &a, const vector<string> &b) { return a[2] < b[2]; });
        for (auto order : orders)
        {
