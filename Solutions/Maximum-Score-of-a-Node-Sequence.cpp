  ...
  public:
    ...
    int maximumScore(vector<int> &scores, vector<vector<int>> &edges)
    {
        ...
        for (const vector<int> &edge : edges)
        {
            ...
            while (top3nodes[a].size() > 3)
                top3nodes[a].pop_back();

            top3nodes[b].push_back(node(a, scores[a]));
            sort(top3nodes[b].begin(), top3nodes[b].end(), greater<node>());
            while (top3nodes[b].size() > 3)
                top3nodes[b].pop_back();
        };
            ...
            for (const node &a_another : top3nodes[a])
                if (b != a_another.another_edge)
                    for (const node &b_another : top3nodes[b])
                        ...
                        {
                            // cout << a << ' ' << b << ' ' << a_another.another_edge << ' ' << b_another.another_edge
                            // << endl;
                            ans = max(ans, scores[a] + scores[b] + scores[a_another.another_edge] +
                                               scores[b_another.another_edge]);
                        };
        };
        return ans;
    }
