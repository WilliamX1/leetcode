...
class Solution
{
  public:
    ...
    int getsum(node *root, int fathercolor, int fathertime)
    {
        ...
        int ans = 0;
        // cout << root->treenode->val << ' ' << root->color << ' ' << root->time << ' ' << root->rightcolor << ' ' <<
        // root->righttime << endl; if (fathertime > root->time) {
        //     root->color = fathercolor;
        //     root->time = fathertime;
        // };

        return root->color + getsum(root->left, root->leftcolor, root->lefttime) +
               getsum(root->right, root->rightcolor, root->righttime);
    };
