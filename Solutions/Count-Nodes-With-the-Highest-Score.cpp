...
{
  public:
    int get(const int &root, const vector<int> &left, const vector<int> &right, vector<int> &leftsons,
            vector<int> &rightsons)
    {
        if (left[root] == -1 && right[root] == -1)
            return 1;

        if (left[root] != -1)
            leftsons[root] = get(left[root], left, right, leftsons, rightsons);
        if (right[root] != -1)
            rightsons[root] = get(right[root], left, right, leftsons, rightsons);
