...
public:
    int maxNumEdgesToRemove(int n, vector<vector<int>>& edges) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            {
            case 1: 
                if (!uf_Alice.unite(edges[i][1], edges[i][2])) res++;
                break;
            case 2: break;
            case 3:
                if (!uf_Alice.unite(edges[i][1], edges[i][2])) uf_Alice_surplus.insert(i);
                break;
            default:
                break;
            } 
        }
        if (uf_Alice.setCount != 2) return -1;
        /* 对Bob */
        ...
        for (int i = 0; i < m; i++) {
            ...
            case 1: 
                break;
            case 2: 
                if (!uf_Bob.unite(edges[i][1], edges[i][2])) res++;
                break;
            case 3:
                if (!uf_Bob.unite(edges[i][1], edges[i][2])) uf_Bob_surplus.insert(i);
                break;
            default:
                break;
            } 
        };
        if (uf_Bob.setCount != 2) return -1;
