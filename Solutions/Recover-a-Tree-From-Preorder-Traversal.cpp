...
public:
    ...
    TreeNode* recoverFromPreorder(string traversal) {
        int idx = 0;
        while (idx < traversal.size() && traversal[idx] != '-') idx++;
        ...
        while (idx < traversal.size()) {
            ...
            while (!stk.empty() && stk.top().level - level != -1) stk.pop();
            if (stk.top().tnode->left == nullptr) stk.top().tnode->left = tn;
            else stk.top().tnode->right = tn;
