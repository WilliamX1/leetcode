...
int balance::CheckBalance()
{
	...
	while (LastChar = GetNextSymbol()) {    //从文件中读入括号，直到文件结束
		switch (LastChar) {
		case'(':case'[':case'{':    //若是3中开括号之一，则进栈
			node.Token = LastChar;
			node.TheLine = currentLine;
			st.push(node);
			break;
		case')':case']':case'}':
			if (st.isEmpty()) {    //无对应的左括号
				++Errors;
	...
	while (!st.isEmpty()) {//栈中的开括号均没有遇到闭括号
		++Errors;
		node = st.pop();
...
char balance::GetNextSymbol()
{
	...
	while (ch == NextChar())
	{
		if (ch == '/') {                          //可能是注释
			...
			else if (ch == '/')  SkipComment(SlashStar);  //处理C++风格的注释
			else PutBackChar(ch);    //不是注释
		}
		else if (ch == '\\' || ch == '"') SkipQuote(ch); //处理字符常量或字符串常量
...
{
	char ch;
	if ((ch = fin.get()) == EOF) return NULL;  //文件读入结束，返回NULL;
	if (ch == '\n') ++currentLine;             //读入换行符'\n',行计数器加1；
	return ch;
}
