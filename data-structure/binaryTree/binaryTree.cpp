...
#include <iostream>
using namespace std;
template <class T> T binaryTree<T>::parent(T x, T flag) const
{
    return flag;
}
...
template <class T> void binaryTree<T>::clear(binaryTree<T>::Node *&t)
{
    if (t == NULL)
        return;
    clear(t->left);
...
template <class T> void binaryTree<T>::preOrder(binaryTree<T>::Node *t) const
{
    if (t == NULL)
        return;
    cout << t->data << ' ';
...
template <class T> void binaryTree<T>::postOrder(binaryTree<T>::Node *t) const
{
    if (t == NULL)
        return;
    postOrder(t->left);
...
template <class T> void binaryTree<T>::midOrder(binaryTree<T>::Node *t) const
{
    if (t == NULL)
        return;
    midOrder(t->left);
...
template <class T> typename binaryTree<T>::Node *binaryTree<T>::find(T x, binaryTree<T>::Node *t) const
{
    Node *tmp;
    if (t == NULL)
        return NULL;
...
template <class T> int binaryTree<T>::size(binaryTree<T>::Node *t) const
{
    if (t == NULL)
        return 0;
    return 1 + size(t->left) + size(t->right);
}
template <class T> int binaryTree<T>::height() const
{
    return height(root);
}
template <class T> int binaryTree<T>::height(binaryTree<T>::Node *t) const
{
    if (t == NULL)
        return 0;
    else
...
template <class T> void binaryTree<T>::_preOrder() const
{
    linkStack<Node *> s;
    Node *current;
...
//中序遍历非递归
template <class T> void binaryTree<T>::_midOrder() const
{
    linkStack<StNode> s;
    StNode current(root);
...
//后续遍历非递归
template <class T> void binaryTree<T>::_postOrder() const
{
    linkStack<StNode> s;
    StNode current(root);
